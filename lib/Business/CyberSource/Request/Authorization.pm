...
sub submit {
	...
	my $res
		= Business::CyberSource::Response::Authorization->new({
			...
			decision       => $ret->valueof('//c:decision'           ),
			reason_code    => $ret->valueof('//reasonCode'           ),
			request_token  => $ret->valueof('//c:requestToken'         ),
			amount         => $ret->valueof('//c:amount'               ),
			auth_code      => $ret->valueof('//c:authorizationCode'    ),
			avs_code_raw   => $ret->valueof('//c:avsCodeRaw'           ),
